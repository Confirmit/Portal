...
namespace Controls.ExtenderBase
{
    internal struct ResourceEntry
    {
        ...
        {
            return new ScriptReference {Assembly = AssemblyName, Name = ResourcePath};
        }
    ...
    public static class ScriptObjectBuilder
    {
        ...
        static ScriptObjectBuilder()
        {
            ...
            Converter<object, string> dateTimeConverter = (value =>
                                                               {
        ...
        private static readonly Dictionary<Type, Converter<object, string>> _customConverters = new Dictionary<Type, Converter<object, string>>();
        ...
        #region [ ScriptReference support ]
        ...
        /// <returns></returns>
        private static IEnumerable<ScriptReference> scriptReferencesFromResourceEntries(ICollection<ResourceEntry> entries)
        {
    	...
    	private static IEnumerable<string> getCssReferences(Control control, Type type, Stack<Type> typeReferenceStack)
        {
            ...
            try
            {
                lock (_sync)
                {
                    ...
                    requiredScripts.Sort((left, right) => left.LoadOrder.CompareTo(right.LoadOrder));
                    ...
                    entries.Sort((l, r) => l.Order.CompareTo(r.Order));
        ...
        public static void RegisterCssReferences(Control control)
        {
            ...
            foreach (string styleSheet in GetCssReferences(control))
            {
                if (header == null)
                {
                ...
                HtmlLink link = new HtmlLink {Href = styleSheet};
                ...
                if (null == scriptManager)
                    throw new InvalidOperationException("Cannot find ScriptManager on the Page.");

                if (scriptManager.IsInAsyncPostBack &&
                    control.Page.Request.Form["__AjaxControlToolkitCalendarCssLoaded"] == null)
                {
