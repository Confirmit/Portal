    ...
    public partial class BookPage : ContentControl
    {
        private static PageParameters ResetPage(UIElement source, CornerOrigin origin)
        {
            PageParameters parameters = new PageParameters(source.RenderSize)
                                            {
                                                Page0ShadowOpacity = 0,
                                                Page1ClippingFigure = new PathFigure(),
                                                Page1ReflectionStartPoint = new Point(0, 0),
                                                Page1ReflectionEndPoint = new Point(0, 0),
                                                Page1RotateAngle = 0,
                                                Page1RotateCenterX = 0,
                                                Page1RotateCenterY = 0,
                                                Page1TranslateX = 0,
                                                Page1TranslateY = 0,
                                                Page2ClippingFigure = new PathFigure(),
                                                Page0ShadowStartPoint = new Point(0, 0),
                                                Page0ShadowEndPoint = new Point(0, 0)
                                            };
        ...
        private static PageParameters? ComputePage(UIElement source, 
                                                   Point p, 
                                                   CornerOrigin origin)
        {
            ...
            // x = k * y + b
            double k = -(yc - p.Y) / (xc - p.X);
            double b = (xc + p.X) / 2 - k * ((yc + p.Y) / 2);
            ...
            _parameters.Page2ClippingFigure.StartPoint = new Point(xc - _parameters.Page1ClippingFigure.StartPoint.X,
                                                                   _parameters.Page1ClippingFigure.StartPoint.Y);
            ...
            LinearGradientHelper.ComputePointsFromTop(xc, yc,
                                                      xc - l1, yc - h1,
                                                      20, 20,
                                                      oppositeOrigin,
            ...
            LinearGradientHelper.ComputePoints(xc, yc,
                                               xc - l1, yc - h1,
                                               r1, r2,
                                               origin,
                ...
                case CornerOrigin.TopRight:
                    if (origin == CornerOrigin.TopRight)
                    {
                        ...
                        else
                        {
                            double d = Math.Sqrt(p.X * p.X + p.Y * p.Y);
                            double R =
                                Math.Sqrt(source.RenderSize.Width*source.RenderSize.Width +
                                          source.RenderSize.Height*source.RenderSize.Height);
