...
namespace ConfirmIt.PortalLib.BusinessObjects.RuleEnities.Executors
{
    ...
    {
        private readonly IActiveStateUserRecognizer _eventTypeRecognizer;
        private readonly IRuleRepository _ruleRepository;
       
        public MessageHelper MessageHelper { get; set; }
        public int UserId { get; set; }

        public NotifyLastUserExecutor(IActiveStateUserRecognizer eventRecognizer, IRuleInstanceRepository ruleInstanceRepository, MessageHelper messageHelper, int userId)
        ...
        private int GetActiveUsersCount(IEnumerable<Person> users)
        {
            ...
            {
                if (_eventTypeRecognizer.IsActive(user.ID.Value))
                    countActiveUser++;
            }
