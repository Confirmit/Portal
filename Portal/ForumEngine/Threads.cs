...
namespace AspNetForums {
    ...
    public class Threads {
        ...
        // ***********************************************************************/
        public static ThreadCollection GetAllThreads(int forumID, int pageSize, int pageIndex, DateTime endDate, string username, bool unreadThreadsOnly, int SortBy, int SortOrder)
        {
            ...
            // Get the threads
            threads =  dp.GetAllThreads(forumID, pageSize, pageIndex, endDate, username, unreadThreadsOnly, SortBy, SortOrder);			
        ...
        // ***********************************************************************/
        public static ThreadCollection GetThreadsUserMostRecentlyParticipatedIn(string username, int count) {
            // Create Instance of the IDataProviderBase
            IDataProviderBase dp = DataProvider.Instance();

            return dp.GetThreadsUserMostRecentlyParticipatedIn(username, count);
        }
        ...
        //
        /// <summary>
        /// Returns threads that the user has tracking enabled for.
        /// </summary>
        /// <param name="username">Username to get tracked posts for</param>
        ...
        // *********************************************************************
        //  GetTotalPinnedPostsForThread
        //
        /// <summary>
        /// Total number of pinned posts in a given thread
        /// </summary>
        /// <param name="postID">Id of the post to find replies</param>
        /// <returns>Total pinned posts in the thread</returns>
        // ***********************************************************************/
        public static int GetTotalPinnedPostsForThread(int postID)
        {

            int postCount = 0;

            // Only do this once per request
            if (HttpContext.Current.Items["TotalPinnedPostsForThread-" + postID] != null)
            {

                postCount = Convert.ToInt32(HttpContext.Current.Items["TotalPinnedPostsForThread-" + postID]);
            ...
            else
            {
                ...
                postCount = dp.GetTotalPinnedPostsForThread(postID);

                HttpContext.Current.Items["TotalPinnedPostsForThread-" + postID] = postCount;
            }
